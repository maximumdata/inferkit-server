...
router.post('/', async (req, res, next) => {
    ...
    const { body: { input, key } } = req;
    if (key !== currentKey) {
        return res.status(401).json({ msg: 'Key is missing or incorrect. Say !api in the bloard discord to get the current key'});
    }
    if (!input) {
        const err = new Error('Missing input in request');
        return res.status(400).json({ err, msg: err.msg || 'You didn\'t send any input!' });
    }
    try {
